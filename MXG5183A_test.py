...
import pyvisa # Working with GPIB control (sorry Dar)
...
class MXG5183A(VisaObject):
    ...
    def __init__(self, NetworkAddress, resourceManager = None):
        ...
        Connecting to the VNA pyvisa driver
        Input: unicode string with pyvisa address
    ...
    def on(self):
        self.pyvisa.write('OUTP:STAT ON')
    def off(self):
        self.pyvisa.write('OUTP:STAT OFF')
    def get_output_state(self):
        self.pyvisa.write('OUTP:STAT?')
    ...
    def atten_prot(self, state):
        ...
        self.pyvisa.write(f':SOUR:SWE:ATT:PROT:STAT {state}')
   
    """19.8.20 Eliya removed Hz from some functions"""
    def set_freq(self, freq): 
        self.pyvisa.write('FREQ:CW '+str(in_hz(freq)))
    def get_freq(self):
        """If no units are given on the input, assumes units of GHz"""
        self.pyvisa.write('FREQ:CW?')
    ...
    def set_freq_start(self,pt_start=None):
        ...
        if pt_start is None:
            self.pyvisa.write(':FREQ:START?')
            return self.read_float()
        if type(pt_start)==Value:
            self.pyvisa.write(':FREQ:START {0}{1}'.format(pt_start[pt_start.units],pt_start.units))
        else:
            self.pyvisa.write(f':FREQ:START {pt_start}Hz')
    def set_freq_stop(self,pt_stop=None):
        ...
        if pt_stop is None:
           self.pyvisa.write(':FREQ:STOP?')
           return self.read_float()
        if type(pt_stop)==Value:
            self.pyvisa.write(':FREQ:STOP {0}{1}'.format(pt_stop[pt_stop.units],pt_stop.units))
        else:
            self.pyvisa.write(f':FREQ:STOP {pt_stop}Hz')
    ...
    def set_power(self, power):
        ...
        self.pyvisa.write('POW:AMPL '+str(in_dbm(power)))
    def get_power(self):
        self.pyvisa.write('POW:AMPL?')
    ...
    def set_pwr_start(self,pt_start=None):
        ...
        if pt_start is None:
            self.pyvisa.query(':POW:START?')
            return self.read_float()*dBm
        if type(pt_start) == Value:
            self.pyvisa.write(':POW:START {0}{1}'.format(pt_start[pt_start.units],pt_start.units))
        else:
            self.pyvisa.write(f':POW:START {pt_start}dBm')
    def set_pwr_stop(self,pt_stop=None):
        """This command sets the last frequency point in a step sweep
        pt_stop is a labrad unit of  power"""
        if pt_stop is None:
            self.pyvisa.query(':POW:STOP?')
            return self.read_float()*dBm
        if type(pt_stop) == Value:
            self.pyvisa.write(':POW:STOP {0}{1}'.format(pt_stop[pt_stop.units],pt_stop.units))
        else:
            self.pyvisa.write(f':POW:STOP {pt_stop}dBm')
    ...
    def attenuator_protection(self,state =0):
        ...
        self.pyvisa.write(':SWE:ATT:PROT {0}'.format(state))
    def Num_SWE_PT(self,Num_of_points=None):
        """This command defines the number of step sweep points"""
        if Num_of_points is None:
            return int(self.pyvisa.query(':SWE:POIN?'))
        self.pyvisa.write(':SWE:POIN {0}'.format(Num_of_points))
    ...
    def set_SWE_SPAC(self,space = None):
        ...
        if space is None:
            return self.pyvisa.query(':SWE:SPAC?')
        self.pyvisa.write(':SWE:SPAC {0}'.format(space))
    def sweep_mode(self, SWE_MOD= None):
        ...
        if SWE_MOD is None:
            return self.pyvisa.query(':SOUR:FREQ:MODE?;:SOUR:POW:MODE?')
        self.pyvisa.write(sweep_mode_dict[SWE_MOD])

    def set_list_sweep(self, freqs, powers, dwells, atten_hold = True):
        if atten_hold:
            self.pyvisa.write(':SOUR:SWE:ATT:PROT:STAT ON')
        self.pyvisa.write(':SOUR:LIST:FREQ '+','.join([str(f) for f in freqs]))
        self.pyvisa.write(':SOUR:LIST:POW '+','.join([str(p) for p in powers]))
        self.pyvisa.write(':SOUR:LIST:DWEL '+','.join([str(d) for d in dwells]))
    ...
    def set_sweep_type(self, sweep_type):
        ...
        self.pyvisa.write(f':LIST:TYPE {sweep_type}')
    def set_dwell(self, time=None):
        ...
        if time is None:
            return self.pyvisa.query('SOUR:SWEep:DWEL?')
        if type(time) == Value:
            self.pyvisa.write('SOUR:SWE:DWEL {0}'.format(time['s']))
        else:
            self.pyvisa.write(f'SOUR:SWE:DWEL {time}')
    """This command sets the dwell time for the current list sweep points The variable is expressed in units of seconds with a 0.000001 (mS)."""
    ...
    def display(self, remote= 'ON'):
        ...
        self.pyvisa.write(':DIS:REM {0}'.format(remote))
    ...
    def set_Sweep_step(self,step=None):
        ...
        self.pyvisa.write(':SWE:STEP: {0}{1}'.format(step[step.units],step.units))
    def EXT_trig_source(self,outport = 'TRIG2'):
        ...
        self.pyvisa.write(':TRIG:EXT:SOUR {0}'.format(outport))
    ...
    def start_sweep(self, typ = 'CONT'):
        ...
        self.pyvisa.write(start_sweep_dict[typ])
    def abort_sweep(self):
        self.pyvisa.write(':INIT:CONT 0;:ABOR')
    ...
    def triger_source(self,SOUR = 'IMM'):
        ...
        self.pyvisa.write(':LIST:TRIG:SOUR {0}'.format(SOUR))
